/*****************************************************************************
 * @file    adc.c
 * @brief   ESP32-C6 ADC Implementation for EMIC SDK
 * @author  EMIC Team
 * @version v1.0.0
 * @date    02/07/2026
 *
 * @details ADC implementation for ESP32-C6 using ESP-IDF adc_oneshot driver.
 *          Note: ESP32-C6 only has ADC1 with 7 channels (GPIO 0-6).
 *          There is NO ADC2 on ESP32-C6.
 *****************************************************************************/
...
/* Channel configuration tracking for ESP32-C6 (7 channels on ADC1 only) */
static uint8_t adc1_atten[7] = {ADC_ATTEN_11dB, ADC_ATTEN_11dB, ADC_ATTEN_11dB,
                                 ADC_ATTEN_11dB, ADC_ATTEN_11dB, ADC_ATTEN_11dB,
                                 ADC_ATTEN_11dB};
static bool adc1_chan_configured[7] = {false};

/*============================================================================
 * ESP32-C6 GPIO to Channel Mapping
 * ADC1: GPIO 0-6 -> Channel 0-6
 * ADC2: Not available on ESP32-C6
 *============================================================================*/

static const int8_t adc1_ch_to_gpio[] = {0, 1, 2, 3, 4, 5, 6};
...
{
    if (unit != 1) {
        ESP_LOGE(TAG, "ESP32-C6 only has ADC1");
        return false;
    }
    ...
    ESP_LOGW(TAG, "ADC1 calibration not available");
    return false;
}
...
bool ADC_initUnit(uint8_t unit)
{
    if (unit == ADC_UNIT_1) {
        ...
        ESP_LOGI(TAG, "ADC1 initialized (7 channels: GPIO 0-6)");
        return true;
    }
    else if (unit == ADC_UNIT_2) {
        ESP_LOGE(TAG, "ESP32-C6 does not have ADC2");
        return false;
    }
...
bool ADC_configChannel(uint8_t unit, uint8_t channel, uint8_t atten)
{
    if (unit == ADC_UNIT_1) {
        ...
        if (channel >= 7) return false;  /* ESP32-C6 has 7 ADC1 channels */
...
bool ADC_configGpio(uint8_t gpio, uint8_t atten)
{
    ...
    if (!ADC_gpioToChannel(gpio, &unit, &channel)) {
        return false;
    }
...
int ADC_rawToVoltage(uint8_t unit, uint8_t channel, int raw_value)
{
    ...
    if (adc1_cali_handle != NULL) {
        esp_err_t ret = adc_cali_raw_to_voltage(adc1_cali_handle, raw_value, &voltage);
        if (ret == ESP_OK) {
            return voltage;
        }
    }
...
{
    /* ESP32-C6 ADC1: GPIO 0-6 -> Channel 0-6 */
    if (gpio <= 6) {
        *unit = ADC_UNIT_1;
        *channel = gpio;
        return true;
