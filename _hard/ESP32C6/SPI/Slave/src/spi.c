 ...
 * @brief   ESP32-C6 SPI.{host}. Slave Implementation for EMIC SDK
 * @author  EMIC Team
 * @version v1.0.0
 * @date    02/07/2026
 *****************************************************************************/
...
static const char *TAG = "SPI.{host}._Slave_C6";
...
void SPI.{host}._Slave_initEx(int mosiPin, int misoPin, int clkPin, int csPin) {
    ...
    spi_bus_config_t bus_config = { .mosi_io_num = mosi_pin, .miso_io_num = miso_pin, .sclk_io_num = clk_pin, .quadwp_io_num = -1, .quadhd_io_num = -1, .max_transfer_sz = SPI.{host}._SLAVE_BUFFER_SIZE };
    spi_slave_interface_config_t slave_config = { .mode = 0, .spics_io_num = cs_pin, .queue_size = 3, .post_trans_cb = spi_slave_post_trans_cb };
    ...
    if (spi_slave_initialize(SPI.{host}._SLAVE_HOST, &bus_config, &slave_config, SPI_DMA_CH_AUTO) != ESP_OK) { ESP_LOGE(TAG, "Failed to initialize SPI slave"); return; }
    slave_initialized = true;
    ESP_LOGI(TAG, "SPI.{host}. Slave initialized");
}

void SPI.{host}._Slave_deinit(void) { if (!slave_initialized) return; spi_slave_free(SPI.{host}._SLAVE_HOST); slave_initialized = false; transaction_callback = NULL; }
