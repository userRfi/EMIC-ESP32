/*****************************************************************************
 * @file    I2C.{port}..c
 * @brief   ESP32 I2C.{port}. Master Implementation for EMIC SDK
 * @author  EMIC Team
 * @version v1.0.0
 * @date    01/31/2026
 *
 * @details I2C Master implementation using ESP-IDF i2c_master driver.
 *****************************************************************************/
...
/* Device handle cache (simple implementation for common use case) */
...
void I2C.{port}._setSpeed(uint32_t clockSpeed)
{
    ...
    /* Clear device cache - new devices will use new speed */
    for (int i = 0; i < cache_count; i++) {
        i2c_master_bus_rm_device(device_cache[i].handle);
    }
    cache_count = 0;
