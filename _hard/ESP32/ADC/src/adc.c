/*****************************************************************************
 * @file    adc.c
 * @brief   ESP32 ADC Implementation for EMIC SDK
 * @author  EMIC Team
 * @version v1.0.0
 * @date    01/31/2026
 *
 * @details ADC implementation using ESP-IDF adc_oneshot driver.
 *****************************************************************************/
...
static uint8_t adc1_atten[8] = {ADC_ATTEN_11dB, ADC_ATTEN_11dB, ADC_ATTEN_11dB, ADC_ATTEN_11dB,
...
static bool adc1_chan_configured[8] = {false};
...
static const int8_t gpio_to_adc1_ch[] = {
    [32] = 4, [33] = 5, [34] = 6, [35] = 7,
    [36] = 0, [37] = 1, [38] = 2, [39] = 3,
};

static const int8_t gpio_to_adc2_ch[] = {
    [0] = 1, [2] = 2, [4] = 0, [12] = 5, [13] = 4,
    [14] = 6, [15] = 3, [25] = 8, [26] = 9, [27] = 7,
};

static const int8_t adc1_ch_to_gpio[] = {36, 37, 38, 39, 32, 33, 34, 35};
static const int8_t adc2_ch_to_gpio[] = {4, 0, 2, 15, 13, 12, 14, 27, 25, 26};
...
void ADC_init(void)
{
    ...
    /* ADC2 is initialized on demand due to WiFi conflict */
}
...
bool ADC_initUnit(uint8_t unit)
{
    if (unit == ADC_UNIT_1) {
        ...
        ESP_LOGI(TAG, "ADC2 initialized (warning: conflicts with WiFi)");
        return true;
    }
...
int ADC_rawToVoltage(uint8_t unit, uint8_t channel, int raw_value)
{
    ...
    /* At 11dB, full scale is approximately 2600mV */
...
{
    /* Check if WiFi is initialized - ADC2 cannot be used with WiFi */
    /* This is a simplified check; in practice, check wifi_init status */
    return true; /* Assume available; actual check depends on WiFi state */
}
