 ...
 * @brief   Common System Implementation for All ESP32 Variants
 * @author  EMIC Team
 * @version v1.0.0
 * @date    02/01/2026
 *
 * @details Implementation of system initialization, timing, and control
 *          functions using ESP-IDF APIs. Works on all ESP32 variants.
 *****************************************************************************/
...
void initSystem(void)
{
    ...
    ESP_LOGI(TAG, "EMIC System initialized on %s", HAL_GetChipModel());
    ESP_LOGI(TAG, "CPU Freq: %lu MHz, Cores: %d, Free Heap: %lu bytes",
             HAL_GetCpuFreqMHz(), HAL_GetCoreCount(), HAL_GetFreeHeap());
}
...
#if ESP32_HAS_USB_SERIAL || ESP32_HAS_USB_OTG
        case ESP_RST_USB:
            return RESET_REASON_USB;
#endif
        default:
