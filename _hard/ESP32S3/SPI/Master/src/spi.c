/*****************************************************************************
 * @file    SPI.{host}..c
 * @brief   ESP32-S3 SPI.{host}. Master Implementation for EMIC SDK
 * @author  EMIC Team
 * @version v1.0.0
 * @date    02/07/2026
 *
 * @details SPI Master implementation for ESP32-S3 using ESP-IDF spi_master driver.
 *          ESP32-S3 has SPI2 and SPI3 available for general use.
 *****************************************************************************/
...
static const char *TAG = "SPI.{host}._S3";
...
{
    /* ESP32-S3 SPI always uses MSB first */
    if (!msbFirst) {
        ESP_LOGW(TAG, "ESP32-S3 SPI only supports MSB first");
    }
}
