/*****************************************************************************
 * @file    wifi.c
 * @brief   ESP32-S3 WiFi Implementation for EMIC SDK
 * @author  EMIC Team
 * @version v1.0.0
 * @date    02/07/2026
 *
 * @details WiFi implementation for ESP32-S3 using ESP-IDF esp_wifi component.
...
bool WiFi_connectTimeout(const char* ssid, const char* password, uint32_t timeout_sec)
{
    ...
    EventBits_t bits = xEventGroupWaitBits(wifi_event_group, WIFI_CONNECTED_BIT | WIFI_FAIL_BIT,
                                            pdFALSE, pdFALSE, pdMS_TO_TICKS(timeout_sec * 1000));
...
{
    if (!buffer) return;
    if (WiFi_isConnected()) sprintf(buffer, IPSTR, IP2STR(&sta_ip_info.ip));
    else strcpy(buffer, "0.0.0.0");
}

void WiFi_getGateway(char* buffer)
{
    if (!buffer) return;
    if (WiFi_isConnected()) sprintf(buffer, IPSTR, IP2STR(&sta_ip_info.gw));
    else strcpy(buffer, "0.0.0.0");
}
...
{
    if (!buffer) return;
    if (WiFi_isConnected()) sprintf(buffer, IPSTR, IP2STR(&sta_ip_info.netmask));
    else strcpy(buffer, "0.0.0.0");
}
...
bool WiFi_startAPEx(const char* ssid, const char* password, uint8_t channel, uint8_t maxConnections)
{
    ...
    wifi_config_t wifi_config = {
        .ap = { .channel = channel, .max_connection = maxConnections,
                .authmode = (password && strlen(password) >= 8) ? WIFI_AUTH_WPA2_PSK : WIFI_AUTH_OPEN }
    };
...
void WiFi_getAPIP(char* buffer)
{
    ...
    else strcpy(buffer, "192.168.4.1");
}
...
{
    return sta_netif && esp_netif_set_hostname(sta_netif, hostname) == ESP_OK;
}

void WiFi_getHostname(char* buffer, size_t bufLen)
{
    if (!buffer || !sta_netif) return;
    const char* hostname = NULL;
    if (esp_netif_get_hostname(sta_netif, &hostname) == ESP_OK && hostname)
