/*****************************************************************************
 * @file    pwm.c
 * @brief   ESP32-S3 PWM (LEDC) Implementation for EMIC SDK
 * @author  EMIC Team
 * @version v1.0.0
 * @date    02/07/2026
 *
 * @details PWM implementation for ESP32-S3 (8 low-speed channels).
 *****************************************************************************/
...
void PWM_init(void)
{
    ...
    ESP_LOGI(TAG, "PWM initialized (ESP32-S3: 8 low-speed channels)");
}
...
int PWM_attach(uint8_t gpio, uint32_t frequency, uint8_t resolution)
{
    ...
    if (resolution < PWM_MIN_RESOLUTION || resolution > PWM_MAX_RESOLUTION) {
        ESP_LOGE(TAG, "Invalid resolution %d (must be 1-14 on S3)", resolution);
        return -1;
    }
    ...
    if (channel < 0) {
        ESP_LOGE(TAG, "No free PWM channels (max 8 on S3)");
        return -1;
    }
