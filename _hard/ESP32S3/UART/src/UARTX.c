/*****************************************************************************
 * @file    UART.{port}..c
 * @brief   ESP32-S3 UART.{port}. Implementation for EMIC SDK
 * @author  EMIC Team
 * @version v1.0.0
 * @date    02/07/2026
 *
 * @details UART implementation for ESP32-S3 using ESP-IDF uart_driver.
 *****************************************************************************/
...
static const char *TAG = "UART.{port}._S3";

/*============================================================================
 * ESP32-S3 Default Pin Configuration
 *============================================================================*/

#if .{port}. == 0
    /* UART0: Default TX=GPIO43, RX=GPIO44 */
    #define UART_TX_PIN     43
    #define UART_RX_PIN     44
#elif .{port}. == 1
    /* UART1: Default TX=GPIO17, RX=GPIO18 */
    #define UART_TX_PIN     17
    #define UART_RX_PIN     18
#elif .{port}. == 2
    /* UART2: Default TX=GPIO19, RX=GPIO20 */
    #define UART_TX_PIN     19
    #define UART_RX_PIN     20
#else
    #error "ESP32-S3 only supports UART0, UART1, and UART2"
#endif
...
void UART.{port}._init(void)
{
    ...
    ESP_LOGI(TAG, "UART.{port}. initialized at %d baud (TX:%d, RX:%d)",
             UART.{port}._BAUD, uart_tx_pin, uart_rx_pin);
}
