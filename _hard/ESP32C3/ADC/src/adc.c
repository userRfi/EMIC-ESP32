/*****************************************************************************
 * @file    adc.c
 * @brief   ESP32-C3 ADC Implementation for EMIC SDK
 * @author  EMIC Team
 * @version v1.0.0
 * @date    02/01/2026
 *
 * @details ADC implementation for ESP32-C3 using ESP-IDF adc_oneshot driver.
 *****************************************************************************/
...
/* Channel configuration tracking for ESP32-C3 */
static uint8_t adc1_atten[5] = {ADC_ATTEN_11dB, ADC_ATTEN_11dB, ADC_ATTEN_11dB,
                                 ADC_ATTEN_11dB, ADC_ATTEN_11dB};
static uint8_t adc2_atten[1] = {ADC_ATTEN_11dB};
static bool adc1_chan_configured[5] = {false};
static bool adc2_chan_configured[1] = {false};

/*============================================================================
 * ESP32-C3 GPIO to Channel Mapping
 * ADC1: GPIO 0-4 -> Channel 0-4
 * ADC2: GPIO 5   -> Channel 0
 *============================================================================*/

static const int8_t adc1_ch_to_gpio[] = {0, 1, 2, 3, 4};
static const int8_t adc2_ch_to_gpio[] = {5};
...
bool ADC_initUnit(uint8_t unit)
{
    if (unit == ADC_UNIT_1) {
        ...
        ESP_LOGI(TAG, "ADC1 initialized (5 channels: GPIO 0-4)");
        return true;
    }
        ...
        ESP_LOGI(TAG, "ADC2 initialized (1 channel: GPIO 5) - conflicts with WiFi");
        return true;
    }
...
bool ADC_configChannel(uint8_t unit, uint8_t channel, uint8_t atten)
{
    if (unit == ADC_UNIT_1) {
        ...
        if (channel >= 5) return false;  /* ESP32-C3 has 5 ADC1 channels */
        ...
        if (!adc2_initialized) {
            ADC_initUnit(ADC_UNIT_2);
        }
        if (channel >= 1) return false;  /* ESP32-C3 has 1 ADC2 channel */
...
{
    /* ESP32-C3 ADC1: GPIO 0-4 -> Channel 0-4 */
    if (gpio <= 4) {
        *unit = ADC_UNIT_1;
        *channel = gpio;
        return true;
    ...
    /* ESP32-C3 ADC2: GPIO 5 -> Channel 0 */
    if (gpio == 5) {
        *unit = ADC_UNIT_2;
        *channel = 0;
        return true;
...
{
    if (unit == ADC_UNIT_1 && channel < 5) {
        return adc1_ch_to_gpio[channel];
    } else if (unit == ADC_UNIT_2 && channel < 1) {
