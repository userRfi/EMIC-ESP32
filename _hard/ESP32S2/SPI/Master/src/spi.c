/*****************************************************************************
 * @file    SPI.{host}..c
 * @brief   ESP32-S2 SPI.{host}. Master Implementation for EMIC SDK
 * @author  EMIC Team
 * @version v1.0.0
 * @date    02/07/2026
 *
 * @details SPI Master implementation for ESP32-S2 using ESP-IDF spi_master driver.
 *****************************************************************************/
...
static const char *TAG = "SPI.{host}._S2";
...
int SPI.{host}._addDevice(int csPin, uint8_t mode)
{
    ...
    int slot = -1;
    for (int i = 0; i < MAX_DEVICES; i++) {
        if (device_handles[i] == NULL) { slot = i; break; }
    }
    if (slot < 0) return -1;
...
{
    if (deviceHandle >= 0 && deviceHandle < MAX_DEVICES && device_handles[deviceHandle] != NULL)
