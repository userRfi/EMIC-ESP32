/*****************************************************************************
 * @file    pwm.c
 * @brief   ESP32-S2 PWM (LEDC) Implementation for EMIC SDK
 * @author  EMIC Team
 * @version v1.0.0
 * @date    02/07/2026
 *
 * @details PWM implementation for ESP32-S2 (8 low-speed channels).
 *****************************************************************************/
...
static bool configure_timer(uint8_t channel, uint32_t frequency, uint8_t resolution)
{
    ...
    ledc_timer_config_t timer_config = {
        .speed_mode = LEDC_LOW_SPEED_MODE,  /* S2 only has low-speed */
        .timer_num = timer,
...
void PWM_init(void)
{
    ...
    ESP_LOGI(TAG, "PWM initialized (ESP32-S2: 8 low-speed channels)");
}
...
int PWM_attach(uint8_t gpio, uint32_t frequency, uint8_t resolution)
{
    ...
    if (resolution < PWM_MIN_RESOLUTION || resolution > PWM_MAX_RESOLUTION) {
        ESP_LOGE(TAG, "Invalid resolution %d (must be 1-14 on S2)", resolution);
        return -1;
    }
    ...
    if (channel < 0) {
        ESP_LOGE(TAG, "No free PWM channels (max 8 on S2)");
        return -1;
    }
