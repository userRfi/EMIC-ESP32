/*****************************************************************************
 * @file    timer.c
 * @brief   ESP32-S2 Timer Implementation for EMIC SDK
 * @author  EMIC Team
 * @version v1.0.0
 * @date    02/07/2026
 *
 * @details Timer implementation for ESP32-S2. 4 hardware timers (2 groups x 2).
 *****************************************************************************/
...
static const char *TAG = "Timer_S2";
...
static hw_timer_slot_t hw_timers[2][2] = {0};
...
bool Timer_start(timer_handle_t handle) {
    ...
    esp_err_t ret = slot->periodic ? esp_timer_start_periodic(slot->handle, slot->period_us)
                                    : esp_timer_start_once(slot->handle, slot->period_us);
...
bool HWTimer_init(int group, int timer, uint16_t divider, bool countUp) {
    if (group < 0 || group > 1 || timer < 0 || timer > 1) return false;
...
void HWTimer_start(int group, int timer) {
    if (group < 0 || group > 1 || timer < 0 || timer > 1) return;
    hw_timer_slot_t* slot = &hw_timers[group][timer];
    if (slot->initialized) gptimer_start(slot->handle);
}

void HWTimer_stop(int group, int timer) {
    if (group < 0 || group > 1 || timer < 0 || timer > 1) return;
    hw_timer_slot_t* slot = &hw_timers[group][timer];
    if (slot->initialized) gptimer_stop(slot->handle);
}

uint64_t HWTimer_getCount(int group, int timer) {
    if (group < 0 || group > 1 || timer < 0 || timer > 1) return 0;
    hw_timer_slot_t* slot = &hw_timers[group][timer];
...
void HWTimer_setCount(int group, int timer, uint64_t value) {
    if (group < 0 || group > 1 || timer < 0 || timer > 1) return;
    hw_timer_slot_t* slot = &hw_timers[group][timer];
    if (slot->initialized) gptimer_set_raw_count(slot->handle, value);
}
